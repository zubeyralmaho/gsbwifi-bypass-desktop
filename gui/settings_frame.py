...
class SettingsFrame(ctk.CTkFrame):
    def __init__(self, master, app, **kwargs):
        ...
        title.grid(row=0, column=0, sticky="w", padx=24, pady=(20, 16))
        ...
        # Auto reconnect
        ctk.CTkLabel(card, text="Otomatik Yeniden Bağlanma", font=ctk.CTkFont(size=13)).grid(
        ...
        self.auto_reconnect_switch.grid(row=row, column=1, sticky="e", padx=20, pady=8)

        row += 1

        # Reconnect interval
        ctk.CTkLabel(card, text="Kontrol Aralığı (sn)", font=ctk.CTkFont(size=13)).grid(
            row=row, column=0, sticky="w", padx=20, pady=8
        )
        ...
        self.interval_label = ctk.CTkLabel(
            interval_frame, text=f"{self.app.settings.get('reconnect_interval')}s",
        ...
        # Login interval
        ctk.CTkLabel(card, text="Giriş İstek Aralığı (sn)", font=ctk.CTkFont(size=13)).grid(
            row=row, column=0, sticky="w", padx=20, pady=8
        )
        ...
        self.login_interval_label = ctk.CTkLabel(
            login_int_frame, text=f"{self.app.settings.get('login_interval')}s",
            font=ctk.CTkFont(size=12), width=35
        )
        ...
        ctk.CTkLabel(card, text="Bilgileri Hatırla", font=ctk.CTkFont(size=13)).grid(
            row=row, column=0, sticky="w", padx=20, pady=8
        )
        self.remember_var = ctk.BooleanVar(value=self.app.settings.get("remember_credentials"))
        self.remember_switch = ctk.CTkSwitch(
            card, text="", variable=self.remember_var,
            command=self._on_remember_change
        )
        self.remember_switch.grid(row=row, column=1, sticky="e", padx=20, pady=8)
    ...
    def _on_auto_reconnect_change(self):
    ...
    def _on_login_interval_change(self, value):
        val = round(value, 1)
