...
class LoginFrame(ctk.CTkFrame):
    def __init__(self, master, app, **kwargs):
        ...
        purpose.pack(fill="x", padx=40, pady=(0, 16))

        # TC Kimlik
        tc_label = ctk.CTkLabel(card, text="TC Kimlik No", anchor="w")
        tc_label.pack(fill="x", padx=40)
        ...
        # Password
        pw_label = ctk.CTkLabel(card, text="Şifre", anchor="w")
        pw_label.pack(fill="x", padx=40)
        ...
        self.remember_cb.pack(padx=40, pady=(0, 16))
        ...
        self.btn_login = ctk.CTkButton(
            ...
            command=self._on_login, fg_color="#2ecc71", hover_color="#27ae60"
        )
        self.btn_login.pack(side="left", padx=(0, 8))

        self.btn_login_warp = ctk.CTkButton(
            btn_frame, text="Bağlan + WARP", width=140,
            command=self._on_login_warp, fg_color="#3498db", hover_color="#2980b9"
        )
        self.btn_login_warp.pack(side="left")

        # Status
        self.status_label = ctk.CTkLabel(
            card, text="", font=ctk.CTkFont(size=12), text_color="#f39c12"
        )
        ...
        if self.app.settings.get("remember_credentials"):
            ...
            saved_pw = self.app.settings.get("saved_password")
            if saved_tc:
    ...
    def _validate(self):
        ...
        if not tc or not sifre:
            self.status_label.configure(text="TC ve Şifre alanları boş bırakılamaz!", text_color="#e74c3c")
            return None, None
        if not tc.isdigit() or len(tc) != 11:
            self.status_label.configure(text="TC Kimlik 11 haneli sayı olmalıdır!", text_color="#e74c3c")
            return None, None
        return tc, sifre
    ...
    def _start_login(self):
        ...
        if self.remember_var.get():
            ...
            self.app.settings.set("saved_password", sifre)
        else:
        ...
        self.status_label.configure(text="Bağlanılıyor...", text_color="#f39c12")
    ...
    def _on_attempt(self, n, msg):
        self.app.after(0, self._update_attempt, n, msg)

    def _on_success(self, n):
        self.app.after(0, self._handle_success, n)

    def _update_attempt(self, n, msg):
        self.status_label.configure(
            text=f"Deneme {n}: {msg}", text_color="#f39c12"
        )
        self.app.log("attempt", f"Deneme {n}: {msg}")

    def _handle_success(self, n):
        if self._progress_active:
            self.progress.stop()
            self.progress.set(1)
            self._progress_active = False
        self.status_label.configure(
            text=f"{n}. denemede bağlantı başarılı!", text_color="#2ecc71"
        )
        ...
        if self._with_warp:
            if not self.app.warp_service.is_installed():
                ...
                self.status_label.configure(text="WARP kuruluyor...", text_color="#f39c12")
                thread = threading.Thread(
    ...
    def _install_and_connect_warp(self):
        ...
        if success:
            ...
            self.app.after(0, self.app.log, level, conn_msg)
        else:
